    ...
    for epoch in range(max_epoch):
        ...
        with tqdm(total=labeled_slice, desc=f'Epoch {epoch + 1}/{max_epoch}', unit='img', leave=is_leave) as pbar:
            for i,batch in enumerate(train_loader):
                ...
                #For CMC
                _,_, masks_pred_mode1, masks_pred_mode2 = cmc_net.forward_CML(imgs_mode1, imgs_mode2)
                ...
                for j,batch_val in enumerate(val_loader):
                    ...
                    if compute_3d==True:
                        ...
                        with torch.no_grad():
                            ...
                            #For CMC
                            _,_,mask_pred_fore_m1, mask_pred_fore_m2 = cmc_net.forward_CML(imgs_fore_m1, imgs_fore_m2)
                            _,_,mask_pred_mid_m1, mask_pred_mid_m2 = cmc_net.forward_CML(imgs_mid_m1, imgs_mid_m2)
                            _,_,mask_pred_after_m1, mask_pred_after_m2 = cmc_net.forward_CML(imgs_after_m1,imgs_after_m2)
                        ...
                        with torch.no_grad():
                            ...
                            #For CMC
                            _, _,mask_pred_mode1,mask_pred_mode2 = cmc_net.forward_CML(imgs_mode1,imgs_mode2)
        ...
        if will_eval == True:
            if epoch < M2_epoch:
                ...
                # logging.info(
                #     'Epoch:[{:0>3}/{:0>3}], Train Loss: {:.4f}, Loss_sup_mode_1_2: {:.4f}, Loss_sup_mode_2_2: {:.4f}, Loss_Teach_M1: {:.4f}, Loss_Teach_M2: {:.4f}, Val Loss: {:.4f}, Train Dice Supervised: mode1 {:.4f} mode2 {:.4f} ,LR: {:.6f}'.format(
                #             epoch,max_epoch,         mean_loss,           mean_loss_sup_mode1_2,    mean_loss_sup_mode2_2,   mean_loss_teach_m1,    mean_loss_teach_m2,    valid_loss_mean,               mean_dice_mode1, mean_dice_mode2, lr_epoch))
                logging.info(
...
def set_argparse():
    ...
    parser.add_argument('--base_dir', type=str,default='res-CMC_STYLE_CML_BraTs-seed-2-10%_seed_1111', 
                        help='base dir name')
