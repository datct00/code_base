...
def train_net(start_time,base_dir,data_path,train_list,val_list,device,img_mode='',
    ...
    #Use for save the var and its value into a dict
    ...
    if class_num==1:
        mask_name='masks' # mask has 1 channel (include all label in 1-channel mask [WT, TC, ET])
    elif class_num==3:
        mask_name='masks_all' # mask has 3 seperated channel for WT, TC and ET 
    ...
    for epoch in range(max_epoch):
        ...
        with tqdm(total=labeled_slice, desc=f'Epoch {epoch + 1}/{max_epoch}', unit='img', leave=is_leave) as pbar:
            for i,batch in enumerate(train_loader):
                ...
                #For CMC
                _,_, masks_pred_mode1, masks_pred_mode2 = cmc_net.forward_mia_no_fuse(imgs_mode1, imgs_mode2)
                ...
                #epoch 52
                    ...
                    for i in range(T):
                        ...
                        with torch.no_grad():
                            # preds[i] = net_mode2_bma(noise_inputs_un)                 #unused [just for syntax]
                            _,_,_, preds_mode_2[i] = cmc_net_bma.forward_mia_no_fuse(noise_inputs_un_mode_2,
                                                                                        #used
                ...
                for j,batch_val in enumerate(val_loader):
                    ...
                    if compute_3d==True:
                        ...
                        with torch.no_grad():
                            ...
                            #For CMC
                            _,_,mask_pred_fore_m1, mask_pred_fore_m2 = cmc_net.forward_mia_no_fuse(imgs_fore_m1, imgs_fore_m2)
                            _,_,mask_pred_mid_m1, mask_pred_mid_m2 = cmc_net.forward_mia_no_fuse(imgs_mid_m1, imgs_mid_m2)
                            _,_,mask_pred_after_m1, mask_pred_after_m2 = cmc_net.forward_mia_no_fuse(imgs_after_m1,imgs_after_m2)
                        ...
                        with torch.no_grad():
                            ...
                            #For CMC
                            _, _,mask_pred_mode1,mask_pred_mode2 = cmc_net.forward_mia_no_fuse(imgs_mode1,imgs_mode2)
...
def set_argparse():
    ...
    parser.add_argument('--base_dir', type=str,default='res-CML_MIA_NO_FUSE_BraTs-seed-2-10%_seed_1111', 
                        help='base dir name')
