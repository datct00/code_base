        ...
        with tqdm(total=labeled_slice, desc=f'Epoch {epoch + 1}/{max_epoch}', unit='img', leave=is_leave) as pbar:
            for i,batch in enumerate(train_loader):
                ...
                if epoch >= start_fusion_epoch: 
                    with torch.no_grad():
                        mode_1_img_F_ds, mode_2_img_F_ds, masks_pred_mode1_unsup, masks_pred_mode2_unsup = cmc_net.forward_onlyCSC_with_sigmoid(imgs_mode1[labeled_bs:], imgs_mode2[labeled_bs:])
