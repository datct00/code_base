...
def train_net(start_time,base_dir,data_path,train_list,val_list,device,img_mode='',
    ...
    def bma_model():
        # Network definitio
        net = CMCNet(image_channels,class_num,32)
        # for param in net.parameters():
        #     param.detach_()
        for name, param in net.named_parameters(): 
            if "encoder_2" in name or "decoder_2" in name: 
                param.detach_()
        return net
    #for CMC
    cmc_net = CMCNet(image_channels, class_num, 32).to(device=device)
    dice_CE_loss = DiceCELoss(to_onehot_y=True, softmax=True, squared_pred=True, smooth_nr=0.0, smooth_dr=1e-6, lambda_dice=1.0, lambda_ce=1.0)
    cmc_net_bma = bma_model().to(device=device)
    ...
    for epoch in range(max_epoch):
        ...
        cmc_net_bma.apply(apply_dropout)
        is_print_negative, is_print_positive = True, True
        with tqdm(total=labeled_slice, desc=f'Epoch {epoch + 1}/{max_epoch}', unit='img', leave=is_leave) as pbar:
            for i,batch in enumerate(train_loader):
                ...
                # only on mode 2
                unlabeled_imgs_mode_1 = imgs_mode1[labeled_bs:]
                unlabeled_imgs_mode_2 = imgs_mode2[labeled_bs:]
                ...
                #For CMC
                _,_, masks_pred_mode1, masks_pred_mode2 = cmc_net.forward_CMC(imgs_mode1, imgs_mode2)
                ...
                #nce_weight = 3.5
                if epoch < M2_epoch:
                    loss_sup_mode1 = dice_CE_loss(masks_pred_mode1[:labeled_bs], true_masks[:labeled_bs])
                    loss_sup_mode2 = dice_CE_loss(masks_pred_mode2[:labeled_bs], true_masks[:labeled_bs])
                    
                    # if is_print_positive:
                    #     print(loss_sup_mode1)
                    #     print(torch.unique(masks_pred_mode1[:labeled_bs]))
                    #     print(torch.unique(true_masks[:labeled_bs]))
                    #     is_print_positive = False
                        
                    loss_sup_all = loss_sup_mode1 + loss_sup_mode2
                    train_log.add_value({"sup_loss": loss_sup_all.item()}, n=1)
                ...
                #epoch 52
                    ...
                    T = T_num
                    _, _, w, h = unlabeled_imgs_mode_2.shape
                    unlabeled_bs = unlabeled_imgs_mode_2.shape[0]
                    
                    # preds_mode_1 = torch.zeros([T,unlabeled_bs, class_num, w, h]).cuda()
                    preds_mode_2 = torch.zeros([T,unlabeled_bs, class_num, w, h]).cuda()
                    
                    for i in range(T):
                        # noise_inputs_un_mode_1 = unlabeled_imgs_mode_1 + \
                        #     torch.clamp(torch.randn_like(
                        #         unlabeled_imgs_mode_1) * 0.1, 0, 0.2)
                        
                        noise_inputs_un_mode_2 = unlabeled_imgs_mode_2 + \
                            torch.clamp(torch.randn_like(
                                unlabeled_imgs_mode_2) * 0.1, 0, 0.2)    
                            
                        with torch.no_grad():
                            # preds[i] = net_mode2_bma(noise_inputs_un)             #unused
                            _,_,_, preds_mode_2[i] = cmc_net_bma.forward_CML(noise_inputs_un_mode_2, 
                                                                             noise_inputs_un_mode_2)
                    # preds = torch.mean(preds, dim=0)
                    
                    # preds_mode_1 = torch.mean(preds_mode_1, dim=0)
                    preds_mode_2 = torch.mean(preds_mode_2, dim=0)


                    # masks_pred_mode1_certainty = preds_mode_1
                    masks_pred_mode2_certainty = preds_mode_2

                    #Interaction between Mode 1 <> Mode 2 unlabeled data
                    ...
                    loss_sup_mode1_2 = dice_CE_loss(masks_pred_mode1[labeled_bs:], masks_pred_mode2_certainty)
                    
                    # if loss_sup_mode1_2 > 0 and is_print_positive:
                    #     print(loss_sup_mode1_2)
                    #     print(torch.unique(masks_pred_mode1[labeled_bs:]))
                    #     print(torch.unique(masks_pred_mode2_certainty))
                    #     is_print_positive = False
                    
                    # if loss_sup_mode1_2 < 0 and is_print_negative: 
                    #     print(loss_sup_mode1_2)
                    #     print(torch.unique(masks_pred_mode1[labeled_bs:]))
                    #     print(torch.unique(masks_pred_mode2_certainty))
                    #     is_print_negative = False
                        
                    loss_contrast_mode1_2 = contrast_loss(masks_pred_mode2_certainty,masks_pred_mode1[labeled_bs:])
                    
                    #Interaction between Mode 2 <> Mode 2 unlabeled data
                    ...
                    loss_sup_mode2_2 = dice_CE_loss(masks_pred_mode2[labeled_bs:], masks_pred_mode2_certainty)
                    ...
                    train_log.add_value({"loss_sup_mode1_2": loss_sup_mode1_2.item()}, n=1)  
                    train_log.add_value({"loss_sup_mode2_2": loss_sup_mode2_2.item()}, n=1) 
                    train_log.add_value({"loss_contrast_mode1_2": loss_contrast_mode1_2.item()}, n=1) 
                    train_log.add_value({"loss_contrast_mode2_2": loss_contrast_mode2_2.item()}, n=1) 
                    train_log.add_value({"loss_teach_m1": loss_teach_m1.item()}, n=1)  
                    train_log.add_value({"loss_teach_m2": loss_teach_m2.item()}, n=1)  
                # # ===========================end for PReL
            ...
            if epoch < M2_epoch: 
                ...
                mean_loss_sup_mode1_2 = train_log.res_dict["loss_sup_mode1_2"][epoch-M2_epoch-1]
                mean_loss_sup_mode2_2 = train_log.res_dict["loss_sup_mode2_2"][epoch-M2_epoch-1]
                mean_loss_contrast_mode1_2 = train_log.res_dict["loss_contrast_mode1_2"][epoch-M2_epoch-1]
                mean_loss_contrast_mode2_2 = train_log.res_dict["loss_contrast_mode2_2"][epoch-M2_epoch-1]
                mean_loss_teach_m1 = train_log.res_dict["loss_teach_m1"][epoch-M2_epoch-1]
                mean_loss_teach_m2 = train_log.res_dict["loss_teach_m2"][epoch-M2_epoch-1]
        ...
        if will_eval == True:
                ...
                for j,batch_val in enumerate(val_loader):
                    ...
                    if compute_3d==True:
                        ...
                        with torch.no_grad():
                            ...
                            #For CMC
                            _,_,mask_pred_fore_m1, mask_pred_fore_m2 = cmc_net.forward_CMC(imgs_fore_m1, imgs_fore_m2)
                            _,_,mask_pred_mid_m1, mask_pred_mid_m2 = cmc_net.forward_CMC(imgs_mid_m1, imgs_mid_m2)
                            _,_,mask_pred_after_m1, mask_pred_after_m2 = cmc_net.forward_CMC(imgs_after_m1,imgs_after_m2)
                        ...
                        with torch.no_grad():
                            ...
                            #For CMC
                            _, _,mask_pred_mode1,mask_pred_mode2 = cmc_net.forward_CMC(imgs_mode1,imgs_mode2)
                    ...
                    loss_val = loss_mode1 + loss_mode2              
            ...
            if T_epoch < epoch < M2_epoch + M2_epoch_all_num:
                ...
                #epoch == 11 + 6 = 17
                    ...
                    update_bma_variables(cmc_net.encoder_2,cmc_net_bma.encoder_2,0.99,1.0)
                    update_bma_variables(cmc_net.decoder_2,cmc_net_bma.decoder_2,0.99,1.0)
                    ...
                    update_bma_variables(cmc_net.encoder_2,cmc_net_bma.encoder_2,0.99,alpha_b)
                    update_bma_variables(cmc_net.decoder_2,cmc_net_bma.decoder_2,0.99,alpha_b)
                    best_performance_m2_list_sort = sorted(best_performance_m2_dict.items(), key=lambda x: x[1])
            # ===========start for PReL
        ...
        if will_eval == True:
            if epoch < M2_epoch:
                logging.info(
                    'Epoch:[{:0>3}/{:0>3}], Train Loss: {:.4f}, Sup loss: {:.4f}, MSE loss: {:.4f}, ASC loss: {:.4f}, Val Loss: {:.4f}, Train Dice Supervised: mode1 {:.4f} mode2 {:.4f} ,LR: {:.6f}'.format(
                            epoch,max_epoch,         mean_loss, mean_mse_loss, mean_sup_loss, mean_asc_loss,    valid_loss_mean,       mean_dice_mode1, mean_dice_mode2, lr_epoch))
            else:
                logging.info(
                    'Epoch:[{:0>3}/{:0>3}], Train Loss: {:.4f}, Loss_sup_mode_1_2: {:.4f}, Loss_sup_mode_2_2: {:.4f}, Loss_contrast_mode1_2: {:.4f}, Loss_contrast_mode2_2: {:.4f}, Loss_Teach_M1: {:.4f}, Loss_Teach_M2: {:.4f}, Val Loss: {:.4f}, Train Dice Supervised: mode1 {:.4f} mode2 {:.4f} ,LR: {:.6f}'.format(
                            epoch,max_epoch,         mean_loss,           mean_loss_sup_mode1_2,    mean_loss_sup_mode2_2,   mean_loss_contrast_mode1_2, mean_loss_contrast_mode2_2, mean_loss_teach_m1,    mean_loss_teach_m2,    valid_loss_mean,               mean_dice_mode1, mean_dice_mode2, lr_epoch))
                # logging.info(
                #     'Epoch:[{:0>3}/{:0>3}], Train Loss: {:.4f},  Val Loss: {:.4f}, Train Dice Supervised: mode1 {:.4f} mode2 {:.4f} ,LR: {:.6f}'.format(
                #             epoch,max_epoch,         mean_loss,          valid_loss_mean,               mean_dice_mode1, mean_dice_mode2, lr_epoch))
        else: 
                logging.info(
                'Epoch:[{:0>3}/{:0>3}], Train Loss: {:.4f}, MSE loss: {:.4f}, ASC loss: {:.4f}, Train Dice Supervised: mode1 {:.4f} mode2 {:.4f} ,LR: {:.6f}'.format(
                        epoch,max_epoch,         mean_loss, mean_mse_loss, mean_asc_loss,       mean_dice_mode1, mean_dice_mode2, lr_epoch))
...
def set_argparse():
    ...
    parser.add_argument('--base_dir', type=str,default='res-TestModeFuseUnLabeled_CML_MIA_BraTs-seed-2-CMC-10%_seed_1111', 
                        help='base dir name')
    ...
    parser.add_argument('--M2_epoch', type=int,default=3,
