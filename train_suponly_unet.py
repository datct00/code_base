    ...
    for epoch in range(max_epoch):
        ...
        with tqdm(total=labeled_slice, desc=f'Epoch {epoch + 1}/{max_epoch}', unit='img', leave=is_leave) as pbar:
            for i,batch in enumerate(train_loader):
                ...
                masks_pred_mode1 = net_mode1(imgs_mode1[:labeled_bs])
                masks_pred_mode2 = net_mode2(imgs_mode2[:labeled_bs])
                ...
                #======================start for only sup
                loss_mode_1 = dice_CE_loss(masks_pred_mode1, true_masks[:labeled_bs])
                loss_mode_2 = dice_CE_loss(masks_pred_mode2, true_masks[:labeled_bs])

                loss_sup_all = (loss_mode_1 + loss_mode_2)/2
...
def set_argparse():
    ...
    parser.add_argument('--base_dir', type=str,default='res-BraTs-seed-2-OnlySup-UNet-10%_seed_1111', 
                        help='base dir name')
