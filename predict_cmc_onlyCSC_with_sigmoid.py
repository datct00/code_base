...
def predict_img(net,device,true_mask,init_img,scale_factor=1,out_threshold=0.5):
    ...
    with torch.no_grad():
        # outs = net(img_tensor)
        _,_,mask_pred_mode_1,mask_pred_mode_2 = net.forward_onlyCSC_with_sigmoid(img_tensor[0], img_tensor[1])
...
def test_volumes(net,device,img_path,true_path,in_files,out_path=None):
    ...
    with torch.no_grad():
        # outs = net(img_tensor)
        _,_,mask_pred_mode_1,mask_pred_mode_2 = net.forward_onlyCSC_with_sigmoid(img_tensor_mode_1, img_tensor_mode_2)
...
if __name__ == "__main__":
    ...
    predict_file = 'res-t2_t1ce-OnlyCSC_CAC_BraTs-seed-2-CMC-withSigmoid-10%_seed_1111/'
