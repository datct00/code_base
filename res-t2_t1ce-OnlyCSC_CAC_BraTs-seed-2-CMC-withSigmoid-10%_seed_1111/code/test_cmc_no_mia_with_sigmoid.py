    ...
    for epoch in range(max_epoch):
        ...
        with tqdm(total=labeled_slice, desc=f'Epoch {epoch + 1}/{max_epoch}', unit='img', leave=is_leave) as pbar:
            for i,batch in enumerate(train_loader):
                ...
                #For CMC
                _,_, masks_pred_mode1_sup, masks_pred_mode2_sup = cmc_net.forward_no_mia_and_fuse_with_sigmoid(imgs_mode1[:labeled_bs], imgs_mode2[:labeled_bs])
                ...
                if epoch >= start_fusion_epoch: 
                    with torch.no_grad():
                        mode_1_img_F_ds, mode_2_img_F_ds, masks_pred_mode1_unsup, masks_pred_mode2_unsup = cmc_net.forward_no_mia_and_fuse_with_sigmoid(imgs_mode1[labeled_bs:], imgs_mode2[labeled_bs:])
                ...
                for j,batch_val in enumerate(val_loader):
                    ...
                    if compute_3d==True:
                        ...
                        with torch.no_grad():
                            ...
                            #For CMC
                            _,_,mask_pred_fore_m1, mask_pred_fore_m2 = cmc_net.forward_no_mia_and_fuse_with_sigmoid(imgs_fore_m1, imgs_fore_m2)
                            _,_,mask_pred_mid_m1, mask_pred_mid_m2 = cmc_net.forward_no_mia_and_fuse_with_sigmoid(imgs_mid_m1, imgs_mid_m2)
                            _,_,mask_pred_after_m1, mask_pred_after_m2 = cmc_net.forward_no_mia_and_fuse_with_sigmoid(imgs_after_m1,imgs_after_m2)
                        ...
                        with torch.no_grad():
                            ...
                            #For CMC
                            _, _,mask_pred_mode1,mask_pred_mode2 = cmc_net.forward_no_mia_and_fuse_with_sigmoid(imgs_mode1,imgs_mode2)
...
def set_argparse():
    ...
    parser.add_argument('--base_dir', type=str,default='res-without_MIA_BraTs-seed-2-CMC-withSigmoid-10%_seed_1111', 
                        help='base dir name')
