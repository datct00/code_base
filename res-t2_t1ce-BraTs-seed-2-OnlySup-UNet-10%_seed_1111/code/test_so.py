    ...
    for epoch in range(max_epoch):
        ...
        with tqdm(total=labeled_slice, desc=f'Epoch {epoch + 1}/{max_epoch}', unit='img', leave=is_leave) as pbar:
            for i,batch in enumerate(train_loader):
                ...
                loss_mode_1 = dice_CE_loss(masks_pred_mode1[:labeled_bs], true_masks[:labeled_bs])
                loss_mode_2 = dice_CE_loss(masks_pred_mode2[:labeled_bs], true_masks[:labeled_bs])
...
def set_argparse():
    ...
    parser.add_argument('--base_dir', type=str,default='res-BraTs-seed-2-OnlySup-Baseline-10%_seed_1111', 
                        help='base dir name')
